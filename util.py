    ...
    def solve(self, mdp, epsilon=0.001):
        ...
        def computeOptimalPolicy(mdp, V):
            ...
            for state in mdp.states:
                pi[state] = max(
                    (computeQ(mdp, V, state, action), action) for action in
                    mdp.actions(state))[1]
            return pi
        ...
        while True:
            newV = {}
            for state in mdp.states:
                # This evaluates to zero for end states, which have no
                # available actions (by definition)
                newV[state] = max(computeQ(mdp, V, state, action) for action in
                                  mdp.actions(state))
            numIters += 1
            if max(abs(V[state] - newV[state]) for state in
                   mdp.states) < epsilon:
                V = newV
                break
            V = newV
...
class MDP:
    ...
    #   state = s, action = a, newState = s', prob = T(s, a, s'), reward =
    #   Reward(s, a, s')
...
# then provide feedback (via incorporateFeedback()) to the RL algorithm,
# so it can adjust
# its parameters.
class RLAlgorithm:
    ...
    # |action| in state |state| resulted in reward |reward| and a transition
    # to state
    # |newState|.
    def incorporateFeedback(self, state, action, reward,
                            newState): raise NotImplementedError("Override me")
